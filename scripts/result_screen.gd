...
func _setup(winner: String) -> void:
	...
	if GameManager.mode == GameManager.GameMode.CAMPAIGN:
		if winner == "player":
			...
			_result_label.modulate = Color(0.3, 1.0, 0.4)
			_detail_label.text = GameManager.campaign_victory_text()
			_play_again_btn.text = "REPLAY CAMPAIGN" if GameManager.campaign_is_final_level() else "NEXT LEVEL"
		else:
			_result_label.text = "DEFEAT"
			_result_label.modulate = Color(1.0, 0.3, 0.2)
			_detail_label.text = "%s holds the line." % GameManager.campaign_opponent_name()
			_play_again_btn.text = "RETRY LEVEL"
		return
	if winner == "player":
		_result_label.text = "VICTORY"
		_result_label.modulate = Color(0.3, 1.0, 0.4)
